...
### Rubrics are user-customizable
The `rubrics/` directory contains domain-specific judge rubrics. Users are expected to add and modify rubrics for their domain. When updating the skill, never overwrite existing rubrics -- only copy rubrics that don't already exist locally.

### No runtime state
This skill has no local state, databases, or credentials stored in the skill directory. Updates are straightforward file copies.

### agent-mux dependency
This skill dispatches parallel judge workers via agent-mux. Ensure agent-mux is installed and configured before using trueskill-rank. Updating trueskill-rank does not affect the agent-mux installation.
